...
def process_multipart(entity):
    ...
    while True:
        ...
        #Inherit maxrambytes from the parent entity.
        #Note that it is per part, not based on the total
        part.maxrambytes = entity.maxrambytes
...
class Entity(object):
    ...
    #Not all Entities use this, but keeping it here simplifies the configurability handling.
    #Normally what will happen is that the Request object will set RequestBody.maxrambytes
    #And the processors will propagate them to child entities.
    maxrambytes = 1000
    ...
    Parts inherit maxrambytes from their parent entity, so this determines
    max per-part size, not max total size.
    """
...
class Part(Entity):
    ...
    def read_into_file(self, fp_out=None):
        """Read the request body into fp_out.

        if fp_out not given, use either make_file or
        a BytesIO object, based on maxrambytes.

        Return fp_out.
        """
        if not fp_out:
            x = self.read_lines_to_boundary()
            if isinstance(x,(str,bytes)):
                return io.BytesIO(x)
            else:
                return x
        else:
            self.read_lines_to_boundary(fp_out)
            return fp_out
...
class SizedReader:
    ...
    def __init__(self, fp, length, maxbytes, bufsize=DEFAULT_BUFFER_SIZE,
        ...
        self.fp = fp
        self.length = length
        if isinstance(maxbytes, tuple):
            self.maxbytes = maxbytes[0]
            self.maxbytes_extend = maxbytes[1]
        else:
            self.maxbytes = maxbytes
            self.maxbytes_extend = None
    ...
    def read(self, size=None, fp_out=None):
        ...
        maxbytes can also be a tuple of (maxbytes, extend_func). If maxbytes
        is exceeded, extend_func will be called, and whatever it returns will be
        the new maxbytes. This will happen once per request.abs

        This allows you to have dynamic upload limits in place before actually
        putting the file on disk or in ram.
        ...
        #Allow maxbytes to be a 2-element tuple of maxbytes, function
        #where function can be called if maxbytes is exceeded and
        #must return a new number to be used as the maxbytes
        #This is so we can have a user defined function to validate uploads before placing them
        #onto disk.
        ...
        if self.buffer:
            ...
            if self.maxbytes and self.bytes_read > self.maxbytes:
                if self.maxbytes_extend:
                    print("extending maxbytes")
                    self.maxbytes = self.maxbytes_extend()
                    #We only want to call maxbytes extend once per request I think
                    self.maxbytes_extend =None
                if self.maxbytes and self.bytes_read > self.maxbytes:
