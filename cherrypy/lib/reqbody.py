...
entities. In short,
:attr:`cherrypy.request.body<cherrypy.lib.request.Request.body>`
is now always set to an instance of
:class:`RequestBody<cherrypy.lib.reqbody.RequestBody>`,
and *that* class is a subclass of :class:`Entity<cherrypy.lib.reqbody.Entity>`.
...
:attr:`request.body.processors<cherrypy.lib.reqbody.Entity.processors>` dict.
If the full media
...
(:func:`default_proc<cherrypy.lib.reqbody.Entity.default_proc>`). For most
...
:attr:`charset<cherrypy.lib.reqbody.Entity.charset>` attribute
...
be used to fill :attr:`name<cherrypy.lib.reqbody.Entity.name>` and
:attr:`filename<cherrypy.lib.reqbody.Entity.filename>` attributes on the
...
it to the :attr:`processors<cherrypy.lib.request.Entity.processors>` dict in a
...
import cherrypy
from cherrypy.lib.compat import basestring
...
class Entity(object):
    ...
    :attr:`entity.parts<cherrypy.lib.reqbody.Entity.parts>`.

    Between the ``before_request_body`` and ``before_handler`` tools, CherryPy
    tries to process the request body (if any) by calling
    :func:`request.body.process<cherrypy.lib.reqbody.RequestBody.process>`.
    This uses the ``content_type`` of the Entity to look up a suitable
    processor in
    :attr:`Entity.processors<cherrypy.lib.reqbody.Entity.processors>`,
    a dict.
    ...
    :func:`default_proc<cherrypy.lib.reqbody.Entity.default_proc>` method
    ...
    :class:`Part<cherrypy.lib.reqbody.Part>` itself if it's a file. (In this
    ...
    :attr:`entity.charset<cherrypy.lib.reqbody.Entity.charset>`. This defaults
    ...
    (see :class:`Part<cherrypy.lib.reqbody.Part>`).
    """
    ...
    def make_file(self):
        ...
        See also :attr:`cherrypy.lib.reqbody.Part.maxrambytes`."""
...
class Part(Entity):
    ...
    :func:`make_file<cherrypy.lib.request.Entity.make_file>`)
