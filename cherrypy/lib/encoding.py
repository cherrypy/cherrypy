...
def gzip_precomp(mime_types=['application/javascript', 'text/html', 'text/plain'],
                 debug=False):
    """Try to find a pre-compressed file, if Content-Type is in mime_types.
    ...
    A pre-compressed file is not used if any of the following hold:
        * The client sends no Accept-Encoding request header
        * No 'gzip' or 'x-gzip' is present in the Accept-Encoding header
        * No 'gzip' or 'x-gzip' with a qvalue > 0 is present
        * The 'identity' value is given with a qvalue > 0.
    ...
    if getattr(request, 'cached', False):
        if debug:
            cherrypy.log('Not searching precomp files in cached response', context='TOOLS.GZIP')
        return
    ...
    if not acceptable:
        ...
        # to the client.
        if debug:
            cherrypy.log('No Accept-Encoding', context='TOOLS.GZIP_PRECOMP')
        return
        ...
        if coding.value in ('gzip', 'x-gzip'):
            ...
            if ct not in mime_types:
                ...
                if not found:
                    if debug:
                        cherrypy.log('Content-Type %s not in mime_types %r' %
                                     (ct, mime_types), context='TOOLS.GZIP_PRECOMP')
                    return
            ...
            root_path = request.config.get('tools.staticdir.root')
            dir_path = request.config.get('tools.staticdir.dir')
            url_path = request.path_info
            static = request.config.get('tools.staticdir.section')
            if url_path.startswith(static):
                url_path = url_path[len(static):]
            if url_path[0] == '/':
                url_path = url_path[1:]
            if root_path and dir_path:
                # request.path_info starts with a '/', thus it can't be part of
                # os.join().
                path = os.path.join(root_path, dir_path,  url_path)
            elif dir_path:
                path = os.path.join(dir_path, url_path)
            path += '.gz'
            ...
            try:
                ...
                if debug:
                    cherrypy.log('os.stat(%r) failed' % path, 'TOOLS.GZIP_PRECOMP')
                return
            # Return a generator that compresses the page
            response.headers['Content-Encoding'] = 'gzip'
            response.body = serve_file(path, content_type=response.headers.get('Content-Type'))
            return
