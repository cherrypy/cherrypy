...
:class:`HTTPError<cherrypy.lib.error.HTTPError>`
or :class:`HTTPRedirect<cherrypy.lib.error.HTTPRedirect>` as the
:attr:`request.handler<cherrypy.lib.request.Request.handler>`.
...
:func:`Request.error_response<cherrypy.lib.request.Request.error_response>` to
set the response status, headers, and body. By default, this is the same
output as
:class:`HTTPError(500) <cherrypy.lib.error.HTTPError>`. If you want to provide
...
:attr:`response.body <cherrypy.lib.request.Response.body>`
...
from cherrypy.lib.compat import basestring, bytestr, iteritems, ntob
from cherrypy.lib.compat import unicodestr, tonative, urljoin as _urljoin
...
class NotFound(HTTPError):
    ...
    :class:`HTTPError("404 Not Found") <cherrypy.lib.error.HTTPError>`.
    """
        ...
        try:
            if hasattr(error_page, '__call__'):
                ...
                if cherrypy.lib.is_iterator(result):
                    from cherrypy.lib.tools.encode import UTF8StreamEncoder
                    return UTF8StreamEncoder(result)
                elif isinstance(result, unicodestr):
                    return result.encode('utf-8')
                else:
                    if not isinstance(result, bytestr):
