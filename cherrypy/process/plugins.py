...
class SignalHandler(object):
    ...
    def __init__(self, bus):
        ...
        if sys.platform[:4] == 'java':
            ...
            self.bus.log(
                'SIGUSR1 cannot be set on the JVM platform. '
    ...
    def _is_daemonized(self):
        ...
        (Buildbot, Jenkins).
        """
        return self._original_pid != os.getpid() and not os.isatty(
            sys.stdin.fileno(),
        )
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                if self.bus:
                    self.bus.log(
                        'Error in background task thread function %r.'
                        % self.function,
                        level=40,
                        traceback=True,
                    )
...
class Monitor(SimplePlugin):
    ...
    def stop(self):
        ...
        if self.thread is None:
            self.bus.log(
                'No thread running for %s.' % self.name
